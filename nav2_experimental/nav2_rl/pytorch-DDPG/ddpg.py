...
class DDPG:
    ...
    def learning(self):
        ...
        a2 = self.actor_target(s2.float()).detach()
        target_q = self.critic_target(s2.float(), a2.float()).detach()
        y_expected = r1[:, None].float() + t1[:, None].float() * self.config.gamma * target_q
        y_predicted = self.critic.forward(s1.float(), a1.float())
        ...
        # actor gradient
        pred_a = self.actor.forward(s1.float())
        loss_actor = (-self.critic.forward(s1.float(), pred_a)).mean()
