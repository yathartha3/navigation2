...
class TurtlebotEnv():
    ...
    def get_reward(self):
        ...
        if self.sq_distance_to_goal() > 0.25:
            distance_reward = (1.0 / (self.sq_distance_to_goal() + 0.0067))  # What's the magic number?
        else:
        ...
        heading_reward = 0.5*cos(self.get_heading())

        # Add the rewards
        reward += distance_reward
        reward += heading_reward

        if self.collision == True:    # this should not be in get_reward(). checked in scan callback
            reward += -100
            self.done = True
